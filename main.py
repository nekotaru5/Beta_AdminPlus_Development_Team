...
updates = [
   {
    ...
    {
        "version": "1.2",
        "add": ["/helpと!helpを追加しました。機能は同じですが、プレフィックコマンドとスラッシュコマンドで実行できます。※誰でも実行可能","/logch: メッセージログを受信するためのコマンドを追加しました。※管理者or許可ロールが必要"],
        "change": ["特に無し"],
        "fix": ["特に無し"]
    },
    {
        "version": "1.1",
        "add": ["アップデート履歴を表示できる機能を追加しました。/updateと!update※誰でも実行可能"],
        "change": ["特に無し"],
        "fix": ["特に無し"]
    },
    {
        "version": "1.0",
        "add": ["🔧 /setbirthdaych [チャンネル]➡ 誕生日通知用チャンネルの登録／解除※管理者 or 許可ロールが必要","📋 /birthdaych_list➡ 登録済みの誕生日通知チャンネルを一覧表示※管理者 or 許可ロールが必要
","👤 /add_birthdaylist [ユーザー] [YYYY-MM-DD]➡ 誕生日を登録！・自分の登録は誰でもOK・他人の登録は管理者 or 許可ロールが必要","🗑️ /delete_birthdaylist [ユーザー]➡ 登録済みの誕生日を削除！・自分の削除は誰でもOK・他人の削除は管理者 or 許可ロールが必要","📆 /birthday_list➡ 登録されている全ユーザーの誕生日を確認※管理者 or 許可ロールが必要
"],
        "change": ["特に無し"],
        "fix": ["特に無し"]
    }
